...
import com.HKSHOPU.hk.databinding.ActivityRecommendAdvertisementBinding
...
class RecommendAdvertisementActivity : BaseActivity() {
    private lateinit var binding: ActivityRecommendAdvertisementBinding
    var MMKV_user_id = MMKV.mmkvWithID("http").getString("UserId", "").toString()
    var shop_id = ""

    private val adapter_product = AdvertisementAdapter("product_recommended")
    private val adapter_store = AdvertisementAdapter("store_recommended")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityRecommendAdvertisementBinding.inflate(layoutInflater)
        ...
        doGetRecommendedAdForProduct(MMKV_user_id, "product")
        doGetRecommendedAdForShop(MMKV_user_id, "shop")
    ...
    private fun initClick() {
        ...
        binding.layoutAddProductAd.setOnClickListener {
            val intent = Intent(this, AddEditProductRecommendedAdvertisementActivity::class.java)
            var bundle = Bundle()
        ...
        binding.layoutAddStoreAd.setOnClickListener {
            val intent = Intent(this, AddEditStoreRecommendedAdvertisementActivity::class.java)
            var bundle = Bundle()
            bundle.putString("shopId", shop_id)
            intent.putExtra("bundle", bundle)
            bundle.putString("mode", "add")
        ...
        binding.layoutProductDropDown.setOnClickListener {
            if(productAdDropDown){
                ...
                valueAnimator.duration = ZoomEngine.DEFAULT_ANIMATION_DURATION
                valueAnimator.start()
...
//                binding.ivStoreDropDown.setRotation(180.0F)
    ...
    private fun doGetRecommendedAdForProduct(user_id: String, ad_type: String) {
        binding.progressBar.visibility = View.VISIBLE
        binding.imgViewLoadingBackground.visibility = View.VISIBLE

        var url = ApiConstants.API_HOST + "user/${user_id}/adSetting/recommend/${ad_type}/"
        val web = Web(object : WebListener {
            ...
            override fun onResponse(response: Response) {
                ...
                try {
                    ...
                    Log.d("doGetRecommendedAdForProduct", "返回資料 resStr：" + resStr)
                    Log.d("doGetRecommendedAdForProduct", "返回資料 ret_val：" + ret_val)

                    val jsonObject: JSONObject = json.getJSONObject("data")
                    Log.d("doGetRecommendedAdForProduct", "返回資料 Object：" + jsonObject.toString())

                    var JSONArray: JSONArray = jsonObject.getJSONArray("productList")
                    if(JSONArray.length()>0){
                        runOnUiThread {
                            binding.layoutProductAdList.visibility = View.VISIBLE
                            binding.layoutProductDropDown.visibility = View.VISIBLE
                        }
                        ...
                        runOnUiThread {
                            renderAdType("product_recommended", list.get(0), list.get(0).status)

                            val layoutManager = LinearLayoutManager(this@RecommendAdvertisementActivity)
                        ...
                        runOnUiThread {
                            ...
                            binding.layoutProductDropDown.visibility = View.GONE
                        }
                    }
    ...
    private fun doGetRecommendedAdForShop(user_id: String, ad_type: String) {
        Log.d("doGetRecommendedAdForShop", "user_id: ${user_id}, ad_type: ${ad_type} ")
        binding.progressBar.visibility = View.VISIBLE
        binding.imgViewLoadingBackground.visibility = View.VISIBLE
        ...
        val web = Web(object : WebListener {
            ...
            override fun onResponse(response: Response) {
                ...
                try {
                    ...
                    val json = JSONObject(resStr)
                    val ret_val = json.get("ret_val")
                    Log.d("doGetRecommendedAdForShop", "返回資料 resStr：" + resStr)
                    Log.d("doGetRecommendedAdForShop", "返回資料 ret_val：" + ret_val)

                    val jsonObject: JSONObject = json.getJSONObject("data")
                    Log.d("doGetRecommendedAdForShop", "返回資料 Object：" + jsonObject.toString())
                    ...
                    if(JSONArray.length()>0){
                      runOnUiThread {
                        ...
                        runOnUiThread {
                            renderAdType("store_recommended", list.get(0), list.get(0).status)
            ...
            override fun onResponse(response: Response) {
                ...
                try {
                    ...
                    if (ret_val.equals("廣告狀態更新成功")) {
                        ...
                        runOnUiThread {
                            Toast.makeText(this@RecommendAdvertisementActivity, ret_val.toString(), Toast.LENGTH_SHORT)
                                .show()
                        }
                        Log.d("doUpdateAdStatus", "ret_val: ${ret_val.toString()}")
                    }
                    ...
                    runOnUiThread {
                        Toast.makeText(this@RecommendAdvertisementActivity, "網路異常", Toast.LENGTH_SHORT).show()
