...
//            activity!!.startActivity(intent)
            ...
            val newFragment: SalesListFragment = SalesListFragment.newInstance(requireFragmentManager())
            val args = Bundle()
//            args.putString("shop_id", shopId)
            newFragment.arguments = args
            ft.replace(R.id.layout_shopinfo, newFragment, "SalesListFragment")
            ft.commit()
        }
...
//            args.putString("shop_id", shopId)
            ...
            Thread(Runnable {
                ...
                RxBus.getInstance().post(EventSaleListToSpecificPage(1))
            }).start()
                ...
                when (it) {
                    ...
                    is EventRefreshShopFucition -> {
                        Thread(Runnable {
                            ...
                            runOnUiThread {
                                var myOderList = MMKV.mmkvWithID("mySaleList").getString("mySaleList", "")
                                Log.d("SaleListFragment_mySaleList", "mySaleList: ${myOderList.toString()}")
                                if(myOderList.equals("SaleListFragment")){
                                    val ft: FragmentTransaction = requireFragmentManager().beginTransaction()
                                    val newFragment: SalesListFragment = SalesListFragment.newInstance(requireFragmentManager())
                                    val args = Bundle()
//                                    args.putString("page_id", "0")
                                    newFragment.arguments = args
                                    ft.replace(R.id.layout_shopinfo, newFragment, "SaleListFragment")
                                    ft.commit()
                                }
                                RxBus.getInstance().post(EventPurchaseListToSpecificPage(0))
                                MMKV.mmkvWithID("mySaleList").putString("mySaleList", "")
                            }
                        }).start()
                    }
