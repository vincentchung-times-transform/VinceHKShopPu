...
//                    Toast.makeText(this@TransactionDetailedInfoForSponsorActivity, """已選取"${mutablelist_addValue.get(position).toString()}"作為付款方式""", Toast.LENGTH_SHORT).show()
//
        ...
        val web = Web(object : WebListener {
            override fun onResponse(response: Response) {
                ...
                try {
                    ...
                    if (status == 0) {
                        val intent = Intent(this@TransactionDetailedInfoForSponsorActivity, FpsPayActivity::class.java)
                        val bundle = Bundle()
//                        bundle.putString("jsonTutList", "[\"${orderId}\"]")
                        intent.putExtra("bundle", bundle)
                        this@TransactionDetailedInfoForSponsorActivity.startActivity(intent)
                    }else if(status == -1) {
                        val intent = Intent(this@TransactionDetailedInfoForSponsorActivity, FpsPayAuditActivity::class.java)
                        val bundle = Bundle()
                        intent.putExtra("bundle", bundle)
                        this@TransactionDetailedInfoForSponsorActivity.startActivity(intent)
                    }
...
//                                ) {
//                                    selected_payment_id = mutablelist_paymentBean.get(position).id
//                                    Toast.makeText(this@TransactionDetailedInfoForSponsorActivity, """已選取"${mutablelist_paymentBean.get(position).payment_desc}"作為付款方式""", Toast.LENGTH_SHORT).show()
//
////                                    var item_id_list = arrayListOf<String>()
////                                    for(i in 0 until mAdapter_ShoppingCartItems.getDatas().size){
////                                        for(j in 0 until mAdapter_ShoppingCartItems.getDatas().get(i).productList.size){
...
//                    }else{
//                        runOnUiThread {
//                            Toast.makeText(this@TransactionDetailedInfoForSponsorActivity , "網路異常請重新連接", Toast.LENGTH_SHORT).show()
//                        }
//                    }
//
//                } catch (e: JSONException) {
