...
class PopularProductAdapter (var currency: Currency, var user_id:String): RecyclerView.Adapter<PopularProductAdapter.TopProductLinearHolder>(){
...
//        val like_click = itemView.find<ImageView>(R.id.iv_product_like_click)
        ...
        fun bindShop(shopPreviewBean : ProductShopPreviewBean){
            ...
            if(shopPreviewBean.product_price.toString() == "-1"){
                price.text = hkd_dollarSign.toString() + shopPreviewBean.min_price.toString() + "-" + shopPreviewBean.max_price.toString()
            }else{
                price.text = hkd_dollarSign.toString() + shopPreviewBean.product_price.toString()
            }
