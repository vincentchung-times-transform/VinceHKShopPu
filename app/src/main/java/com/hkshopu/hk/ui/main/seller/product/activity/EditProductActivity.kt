...
import com.HKSHOPU.hk.ui.main.seller.product.adapter.ShippingFareCheckedAdapter
...
class EditProductActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.titleEditProduct.setText(getText(R.string.title_editproduct))
                    ...
                    if(MMKV_editTextEntryProductDiscription.isNotEmpty()){
                        if(MMKV_proCate_id.isNotEmpty()||MMKV_proSubCate_id.isNotEmpty()){
                            if(MMKV_weight.isNotEmpty() && MMKV_length.isNotEmpty() && MMKV_width.isNotEmpty() && MMKV_height.isNotEmpty()){
                                if( !MMKV_editTextMerchanPrice.toString().equals("") &&
                                    !MMKV_editTextMerchanQunt.equals("") &&
        ...
        if (requestCode == REQUEST_EXTERNAL_STORAGE && resultCode == Activity.RESULT_OK) {
            ...
            Thread(Runnable {
                ...
                if (clipData != null) {
                    ...
                    var sum_pics_number = mutableList_pics.size + clipData.itemCount

                    if(sum_pics_number <= 5){
...
//                                            ItemPics(bitmap, R.mipmap.cover_pic)
                                        )
...
//                                        )
                    ...
                    var sum_pics_number = mutableList_pics.size + 1
...
//                                    ItemPics(bitmap, R.drawable.custom_unit_transparent)
    ...
    fun getResizedBitmapToSquare(image: Bitmap): Bitmap? {
        ...
        return Bitmap.createScaledBitmap(image, width_new, height_new, true)
    }
            ...
            override fun onResponse(response: Response) {
                ...
                try {
                    ...
                    if (ret_val.equals("已取得商品資訊!")) {
                        ...
                        MMKV.mmkvWithID("editPro").putString("inven_quant_range", "${productInfoList.min_quantity}-${productInfoList.max_quantity}")
