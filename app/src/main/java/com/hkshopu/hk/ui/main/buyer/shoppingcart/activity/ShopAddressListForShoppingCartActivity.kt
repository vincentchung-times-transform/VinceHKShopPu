...
import com.HKSHOPU.hk.ui.main.buyer.shoppingcart.adapter.UserAddressItemAdapter
import com.HKSHOPU.hk.ui.main.seller.shop.activity.ShopAddressPresetActivity
...
class ShopAddressListForShoppingCartActivity : BaseActivity() {
    ...
    var user_id = MMKV.mmkvWithID("http").getString("UserId", "").toString()
    ...
    fun initEvent() {
        RxBus.getInstance().toMainThreadObservable(this, Lifecycle.Event.ON_DESTROY)
            .subscribe({
                when (it) {
                    is EventRefreshShoppingCartBuyerAddressList ->{
                        doGetUserAddressList( user_id)
                    }
                }
            }, {
        ...
        val web = Web(object : WebListener {
            override fun onResponse(response: Response) {
                ...
                try {
                    ...
                    if (status == 0) {
                        mutableList_userAddressBean.clear()
